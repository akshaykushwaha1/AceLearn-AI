...
function App() {
  ...
  async function sendChatMessage() {
    ...
    // Snapshot the conversation so far — this is what gets sent as
    // "history" so the AI remembers earlier turns (e.g. "name all the
    // chapters" after discussing a specific book).
    const historyToSend = chatMessages.slice(-16);
    ...
    try {
      ...
      setChatMessages((previous) => [
        ...previous,
        {
          role: "assistant",
          content:
            `Sorry, kuch gadbad ho gayi: ${detail}`,
        },
      ]);
    } finally {
            ...
            <section className="dashboard-card" style={{ marginTop: "18px" }}>
              ...
              {progress.weakTopics.length > 0 && (
                <div style={{ marginTop: "16px" }}>
                  <small style={{ color: "#f87171" }}>NEEDS WORK</small>
                  <div className="decision-tags" style={{ marginTop: "8px" }}>
                    {progress.weakTopics.map((t) => (
                      <span
                        key={t}
                        style={{ background: "rgba(248,113,113,0.12)", color: "#f87171", cursor: "pointer" }}
                        onClick={() => {
                          setTopic(t);
                          startSession();
                        }}
              ...
              {progress.strongTopics.length > 0 && (
                <div style={{ marginTop: "16px" }}>
                  <small style={{ color: "#86efac" }}>STRONG</small>
                  <div className="decision-tags" style={{ marginTop: "8px" }}>
                    {progress.strongTopics.map((t) => (
                      <span key={t} style={{ background: "rgba(134,239,172,0.12)", color: "#86efac" }}>
                        {t}
                      </span>
                    ))}
                  </div>
